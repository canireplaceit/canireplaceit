...
Don't commit images. `bun run icons` fetches them at build time into
`apps/frontend/public/icons/` — product favicons by domain, open source projects by forge
avatar (GitHub, GitLab, Gitea, Forgejo and Codeberg each have their own API path). Everything
is re-encoded to WebP at 96px, lossy and lossless both tried and the smaller one kept.
Missing icons fall back to a lettermark, so a failed fetch is never fatal.
